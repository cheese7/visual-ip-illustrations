...
|---------|-----------|-----|--------|------|----------|-----------|-----------|
| ![Trust Bridge - Xiaohei](../examples/images-en/08-trust-bridge.png) | ![Trust Bridge - Littlebox](../examples/images-en/littlebox/08-trust-bridge.png) | ![Trust Bridge - Tom](../examples/images-en/tom/08-trust-bridge.png) | ![Trust Bridge - Ferris](../examples/images-en/ferris/08-trust-bridge.png) | ![Trust Bridge - Seal](../examples/images-en/seal/08-trust-bridge.png) | ![Trust Bridge - OpenClaw](../examples/images-en/openclaw/08-trust-bridge.png) | ![Trust Bridge - Go Gopher](../examples/images-en/gopher/08-trust-bridge.png) | ![Trust Bridge - Cai Xukun](../examples/images/caixukun/08-trust-bridge.png) |
...
`{visual IP}` 可以是 `Xiaohei`、`Littlebox`、`Tom`、`Ferris`、`Seal`, `OpenClaw`, `Go Gopher`, `Cai Xukun`, `Hermes Agent`, `Linux Mascot` 或支援的別名。省略視覺 IP 時選擇 Xiaohei。
...
protected-character、source-reviewed、active source-history、gated-public-figure 路由會自動帶入 route status、source/rights note、release gate 與特定輸出目錄。Hermes Agent 另會帶入 `Generated image 1 (16).jpeg` uploaded-image authority、official source context、MIT license context、mythology-drift boundary、product-poster boundary、public sample review gate、route isolation、uploaded-character-only article illustration output，以及 `assets/<article-slug>-hermes/`。
...
6. 載入所選 IP 的 canonical pack，組裝 prompts，逐張生成圖像。Mixed-IP requests 會建立分開的 route groups 與 output directories，Xiaohei, Littlebox, Tom, Ferris, Seal, OpenClaw, Go Gopher, Cai Xukun, Hermes Agent, and Linux Mascot each keeping route-local references。
